export const APP_MESSAGES = {
  REQUIRED_FIELD: 'This field is required.',
  INVALID_EMAIL: 'Please enter a valid email address.',
  PASSWORD_LENGTH: 'Password should be at least 6 characters long.',
  PASSWORD_REQUIRED: 'Password is required.',
  CHECK_EMAIL_PASSWORD: 'Check email or password.',
  CONTACT_SUCCESS: 'Message sent successfully.',
  CONTACT_ERROR: 'Error sending your message.',
  UPDATE_EMAIL_SUCCESS: 'Email updated successfully.',
  ERROR_TO_UPDATE_EMAIL: 'Error updating email.',
  UPDATE_NAME_SUCCESS: 'Name updated successfully.',
  ERROR_TO_UPDATE_NAME: 'Error updating name.',
  UPDATE_PASSWORD_SUCCESS: 'Password updated successfully.',
  ERROR_TO_UPDATE_PASSWORD: 'Error updating password.',
  UPDATE_PHONE_SUCCESS: 'Phone number updated successfully.',
  ERROR_TO_UPDATE_PHONE: 'Error updating phone number.'
};
